var petadasar = new L.LayerGroup();

	// var pch = new L.markerClusterGroup();
  //   var pda = new L.markerClusterGroup();
  //   var iklim = new L.markerClusterGroup();
  //   var pks = new L.markerClusterGroup();

    var das = new L.LayerGroup();
    var kipp = new L.LayerGroup();
    var kikn = new L.LayerGroup();
    var kpikn = new L.LayerGroup();

var map = L.map('map', {
	center: [0.198859, 116.731690],
	zoom: 7.2,
	zoomControl:false,
	layers:[]
});
...
// var mapquestPhotoLabel = L.layerGroup([L.tileLayer("http://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}", {
//   maxZoom: 18,
//   reuseTiles: true,
...
var basemaps = {
    ...
    "OSM Color":OpenStreetMap_Mapnik,
  ...
  var groupedOverlays = {
    ...
    '<h6>Sebaran Pos</h6>': {
        ...
        // ' Pos Duga Air': pda,
        // ' Stasiun Iklim': iklim,
        // ' Pos Curah Hujan Duga Air Kerja Sama': pks
    },
    '<h6>Peta Dasar<h6>': {
        ' DAS Sanggai': das,
        ' KIPP': kipp,
...
$.getJSON("../geojson/sanggai.geojson", function(color) {
  L.geoJson(color, {
      style: function(feature) {
          var color = feature.properties.color;
          return { color: "#999", weight: 2, color: color, fillOpacity: .2 };
      },
      onEachFeature: function(feature, layer) {
          layer.bindPopup("<table class='table table-striped table-bordered table-condensed'>" +
              "<tr><th>Nama DAS</th><td>" + feature.properties.Nama_DAS + "</td></tr>" +
              "<tr><th>Wilayah Sungai</th><td>" + feature.properties.WS + "</td></tr>" +
              "<table>")
      }
  }).addTo(das);
});
